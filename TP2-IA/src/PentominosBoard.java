...
public class PentominosBoard implements Comparable<PentominosBoard>{
	...
	boolean putPiece(int p, int row, int col) {  // try to place a piece on the board, return true if it fits
        ...
        ArrayList<Integer> posPair = new ArrayList<Integer>();
        posPair.add(new Integer(row));
        posPair.add(new Integer(col));
	...
	public boolean removePiece(int p) { // Remove piece p from the board, at position (row,col)
        if(this.positions.get(p) == null)
        	return false;
        int row = this.positions.get(p).get(0), col = this.positions.get(p).get(1);
	...
	public int nbrConflict(int c) {
		...
		if(pos == -1) {
			...
			ArrayList<Integer> position = this.positions.get(pos);
			int row = position.get(0), col = position.get(1), diff = 0;
	...
	public PentominosBoard copy() {
		...
		for(int i = 0; i < 63; i++) {
			if(this.positions.get(i) != null)
			copy.positions.set(i, (ArrayList<Integer>)this.positions.get(i).clone());
		}
	...
	public boolean samePositions(PentominosBoard pb) {
		...
		for(int k = 0; k < 63 && isSame; k++) {
			ArrayList<Integer> position1 = this.positions.get(k), position2 = pb.positions.get(k);
			if(position1 != null || position2 != null)
				if(position1 != null && position2 != null)
					isSame = position1.get(0).compareTo(position2.get(0)) == 0 && position1.get(1).compareTo(position2.get(1)) == 0;
				else
					isSame = false;
		}
		return isSame;
